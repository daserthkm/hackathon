...
class DB {
  ...
  fetchManyOrderedByField (table, field, value, orderby, direction = 'ASC') {
    if (direction !== 'ASC') {
      direction = 'DESC'
    }

    orderby = orderby.split(',').map((field) => {
      return `\`${field.trim()}\` ${direction}`
    })

    return new Promise((resolve, reject) => {
      this.context.all(`SELECT * FROM ${table} WHERE \`${field}\` = $value ORDER BY ${orderby.join(',')}`, {
        $value: value
      }, (err, rows) => {
        if (err) {
          reject(err)
  ...
  deleteOneByID (table, id) {
    return new Promise((resolve, reject) => {
      this.context.run(`DELETE FROM ${table} WHERE id = $id`, {
        $id: id
      }, (err) => {
        if (err) {
          reject(err)
  ...
  deleteManyByField (table, field, value) {
    return new Promise((resolve, reject) => {
      this.context.run(`DELETE FROM ${table} WHERE ${field} = $value`, {
        $value: value
      }, (err) => {
        if (err) {
          reject(err)
